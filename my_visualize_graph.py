...
def _visualize_graph_via_graphviz(
    ...
    cmap = cm.get_cmap('viridis')  # 使用viridis颜色映射，你也可以选择其他如'hot', 'cool', 'spring', 'summer', etc.

    for node in edge_index.view(-1).unique().tolist():
        g.node(str(node) if node_labels is None else node_labels[node])

    for (src, dst), w in zip(edge_index.t().tolist(), edge_weight.tolist()):
        color = cmap(norm(w))  # 将权重转换为颜色
        hex_color = matplotlib.colors.rgb2hex(color)  # 转换为十六进制颜色

        if node_labels is not None:
            src = node_labels[src]
            dst = node_labels[dst]
        g.edge(str(src), str(dst), color=hex_color)
